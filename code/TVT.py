...
class TrackingPoint():
    ...
    def update_all_values(self):
        xyt = np.array([xyt for xyt in
                        zip(self.x, self.y, np.arange(len(self.x)))
...
class ThermalVideoModel(QObject):
    ...
    def exportThermalDataVideo(self):
        ...
        ret = QMessageBox.question(
            self.main_win, 'Load exported video?',
    ...
    def sync_video_thermal(self, on_sync):
        ...
        # Set time offest
        t_thermal_ms = 1000 * (self.thermalData.frame_position /
                               self.thermalData.frame_rate)
        t_video_ms = 1000 * (self.videoData.frame_position /
                             self.videoData.frame_rate)
        self.videoData.shift_from_refTime = t_video_ms - t_thermal_ms

        # Set common time range
        dur_thermal_ms = (self.thermalData.duration_frame /
                          self.thermalData.frame_rate) * 1000
        dur_video_ms = (self.videoData.duration_frame /
                        self.videoData.frame_rate) * 1000
        comt_min = int(min(0, -self.videoData.shift_from_refTime))
        comt_max = int(max(dur_thermal_ms,
    ...
    def set_common_time(self, time_ms=None, caller=None):
        ...
        if self.thermalData.loaded:
            if caller != self.thermalData:
                self.thermalData.show_frame(common_time_ms=time_ms,
                                            sync_update=False)
    ...
    def play(self):
        ...
        if self.thermalData.loaded:
            self.play_frame_interval_ms = min(1000/self.thermalData.frame_rate,
                                              self.play_frame_interval_ms)
        ...
        if point_name not in self.tracking_mark.keys():
            ...
            if point_name in self.tracking_point:
                ...
                self.tracking_point[point_name] = \
                    TrackingPoint(self.thermalData, x, y)
    ...
    def load_tracking(self):
        ...
        for point in PointNames:
            ...
            if data_time == 'video':
                ...
                video_times = [self.videoData.get_comtime_from_frame(frmIdx)
    ...
    def load_status(self, fname=None, **kwargs):
        ...
        if 'tracking_point' in settings:
            for lab, dobj in settings['tracking_point'].items():
                ...
                elif dmovie_fname == str(self.videoData.filename) \
...
class MainWindow(QMainWindow):
    ...
    def init_ui_objects(self):
        ...
        self.thermalPositionLab = QLabel(
                '00:00/00:00 [0/0 frames]')
        self.thermalPositionLab.setSizePolicy(QSizePolicy.Preferred,
                                              QSizePolicy.Maximum)
        ...
        self.commonPosisionLab.setSizePolicy(QSizePolicy.Preferred,
                                             QSizePolicy.Maximum)
        ...
        qimg = QImage(frame.data, frame.shape[1], frame.shape[0], bytesPerLine,
    ...
    def connect_signal_handlers(self):
        ...
        self.roi_delete_btn.clicked.connect(
                lambda: self.model.delete_point(point_name=None,
                                                ask_confirm=True))
        self.roi_plot_btn.clicked.connect(partial(self.model.plot_timecourse,
    ...
    def set_layout(self):
        ...
        self.thermal_cbar_lab.setSizePolicy(QSizePolicy.Fixed,
                                            QSizePolicy.Ignored)
        ...
        thermalCtrlUpperLayout.addWidget(self.tmark_grpbx,
        ...
        thermalFrame = QFrame()
        thermalFrame.setSizePolicy(QSizePolicy.Expanding,
        ...
        self.thermalMakerLab.setSizePolicy(QSizePolicy.Expanding,
                                           QSizePolicy.Fixed)
