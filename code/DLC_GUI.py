...
class VideoDataMovie(DataMovie):
    ...
    def show_frame(self, frame_idx=None, common_time_ms=None, sync_update=True):
        super(VideoDataMovie, self).show_frame(frame_idx, common_time_ms, sync_update)
        ...
        if self.model.main_win.plot_xvals is not None:
            ...
            for pp, tl in self.model.main_win.plot_timeline.items():
                if tl.get_xdata()[0] != xpos:
                    if (
                        pp not in self.model.main_win.plot_line
                        or len(self.model.main_win.plot_line[pp]) == 0
                    ):
...
class DLC_GUI(QObject):
    ...
    def select_point_ui(self, point_name=None, update_plot=True):
        ...
        if point_name in self.tracking_mark:
            ...
            self.main_win.roi_rad_spbx.setValue(self.tracking_point[point_name].radius)
    ...
    def edit_tracking_point(self, point_name):
        ...
        if point_name not in self.tracking_mark.keys():
            ...
            if point_name in self.tracking_point:
                ...
                self.tracking_point[point_name] = TrackingPoint(self.videoData, x, y)
                ...
                frame_indices = np.arange(
                    self.videoData.frame_position, self.videoData.duration_frame
                )
                self.tracking_point[point_name].set_position(
                    x, y, frame_indices, update_frames=[self.videoData.frame_position]
                )
    ...
    def show_marker(self):
        ...
        if videoFrmIdx in self.time_marker:
            ...
            self.main_win.videoMakerLab.setStyleSheet("background:red; color:white;")
    ...
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    def plot_timecourse(self, update_val=False, plot_all=False, *args, **kwargs):
        ...
        for pp in ("x", "y"):
            if pp not in self.main_win.plot_timeline:
                self.main_win.plot_timeline[pp] = self.main_win.plot_ax[pp].axvline(
        ...
        for pp in ("x", "y"):
            for point in Points:
                ...
                if plab not in self.main_win.plot_line[pp]:
                    # Create lines
                    self.main_win.plot_line[pp][plab] = self.main_win.plot_ax[pp].plot(
                        self.main_win.plot_xvals, val, "-", color=col, label=plab
                    )[0]
                else:
        ...
        if len(self.main_win.plot_line["x"]):
            self.main_win.plot_ax["x"].legend(
                bbox_to_anchor=(1.001, 1), loc="upper left"
            )
    ...
    def export_roi_data(self, fname=None, **kwargs):
        ...
        if fname is None:
            ...
            initial_name = stdir / (
                self.videoData.filename.stem + "_tracking_points.csv"
            )
        ...
        with open(fname, "w") as fd:
            ...
            fd.write(saveData.to_csv(quoting=csv.QUOTE_NONNUMERIC, encoding="cp932"))
    ...
    def load_tracking(self, fileName=None, lh_thresh=None, **kwargs):
        ...
        for point in PointNames:
            ...
            self.tracking_point[point].set_position(valid_x, valid_y, valid_frms)

            if point in point_property:
                self.tracking_point[point].radius = point_property[point]["radius"]
                self.tracking_mark[point]["rad"] = point_property[point]["radius"]
    ...
    def load_status(self, fname=None, **kwargs):
        ...
        if fname != APP_ROOT / "config" / "DLCGUI_last_working_state-0.pkl":
...
class ViewWindow(QMainWindow):
    ...
    def init_ui_objects(self):
        ...
        self.plot_ax = {
            "xy"[ii]: ax
            for ii, ax in enumerate(self.roi_plot_canvas.figure.subplots(2, 1))
        }
    ...
    def set_layout(self):
        ...
        self.sli_hspace0 = QSpacerItem(12, 1, QSizePolicy.Minimum, QSizePolicy.Minimum)
        slider_hlayout.addSpacerItem(self.sli_hspace0)
        slider_hlayout.addWidget(self.positionSlider)
        self.sli_hspace1 = QSpacerItem(70, 1, QSizePolicy.Minimum, QSizePolicy.Minimum)
    ...
    def make_menu_objects(self):
        ...
        loadVideoAction.triggered.connect(self.model.openVideoFile)
